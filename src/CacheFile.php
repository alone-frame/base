...
class CacheFile{
    ...
    public static function set(string|int $key, mixed $val, int $time = 0): mixed {
        $filePath = static::$filePath ?: alone_root_path('runtime/alone/cache/file');
        Frame::mkDir($filePath);
        $val = (is_callable($val) ? $val() : $val);
        $file = $filePath . '/' . md5($key) . '.cache';
        $fp = fopen($file, 'w');
        if (flock($fp, LOCK_EX)) {
    ...
    public static function lock(string|int $key, callable $callable, callable|bool $closure = false, int $timeout = 5, int $wait = 100): mixed {
        $filePath = static::$lockPath ?: alone_root_path('runtime/alone/cache/lock');
        Frame::mkDir($filePath);
        $fp = fopen($filePath . '/' . $key . '.cache', 'a+');
        if ($fp) {
            $startTime = time();
            while (true) {
    ...
    public static function lockFile(string|int $make, callable $callable): mixed {
        $filePath = static::$lockPath ?: alone_root_path('runtime/alone/cache/lockFile');
        Frame::mkDir($filePath);
        $fp = fopen($filePath . '/' . $make . '.cache', 'a+');
        if (flock($fp, LOCK_EX)) {
            $start = date("Y-m-d H:i:s");
            $res = $callable();
            fwrite($fp, $start . " ~ " . date("Y-m-d H:i:s") . "\r\n");
